...
import { sampleWithRequiredData } from '../check-list.test-samples';
...
describe('CheckList Management Component', () => {
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.checkLists?.[0]).toEqual(expect.objectContaining({ id: 123 }));
  });

  describe('trackId', () => {
    it('Should forward to checkListService', () => {
      const entity = { id: 123 };
      jest.spyOn(service, 'getCheckListIdentifier');
      const id = comp.trackId(0, entity);
      expect(service.getCheckListIdentifier).toHaveBeenCalledWith(entity);
