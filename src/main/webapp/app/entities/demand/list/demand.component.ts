...
import { EntityArrayResponseType, DemandService } from '../service/demand.service';
...
export class DemandComponent implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = ['demandBy', 'status'];
  ...
  trackId = (_index: number, item: IDemand): number => this.demandService.getDemandIdentifier(item);
  ...
  search(query: string): void {
    ...
    if (query && predicate && DemandComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      const { predicate } = this.sortState();
      if (predicate && DemandComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
        this.sortState.set({});
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.demands = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IDemand[] | null): IDemand[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.demandService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.demandService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
